...
TransOnline4 = FOREACH TransOnline3 GENERATE membership_nbr,
					 card_holder_nbr,
...
TransOnline5 = GROUP TransOnline4 BY (membership_nbr, card_holder_nbr, cat_subcat_nbr, visit_date);

TransOnline6 = FOREACH TransOnline5 GENERATE FLATTEN(group) AS (membership_nbr, card_holder_nbr, cat_subcat_nbr, visit_date),
							MAX(TransOnline4.unit_qty) AS unit_qty;
							--SUM(TransOnline4.unit_qty) AS unit_qty;

TransOnline7 = JOIN Mem4 BY (membership_nbr, cardholder_nbr) RIGHT OUTER, TransOnline6 BY (membership_nbr, card_holder_nbr); 


TransOnline8 = FOREACH TransOnline7 GENERATE TransOnline6::membership_nbr AS membership_nbr,
					 (Mem4::membership_create_date IS NULL ? '0000-00-00' : Mem4::membership_create_date) AS membership_create_date,
					 --Mem4::membership_obsolete_date AS membership_obsolete_date,
					 --Mem4::cardholder_nbr AS cardholder_nbr, 
					 TransOnline6::card_holder_nbr AS cardholder_nbr, 
					 TransOnline6::cat_subcat_nbr AS cat_subcat_nbr,
...
TransOnline82 = GROUP TransOnline81 BY (membership_nbr, membership_create_date, cardholder_nbr, cat_subcat_nbr, visit_date);

TransOnline83 = FOREACH TransOnline82 GENERATE FLATTEN(group) AS (membership_nbr, membership_create_date, cardholder_nbr, cat_subcat_nbr, visit_date),
...
TransOffline0 = foreach TransOffline0 generate
	 visit_nbr,
         club_nbr,
         system_item_nbr,
         household_id,
         ...
         category_nbr,
         sub_category_nbr,
         --(double) retail_all / EXP( ( DaysBetween(ToDate('$today'),ToDate(visit_date)))/365.0) as retail_all,
         --(double) unit_qty / EXP( ( DaysBetween(ToDate('$today'),ToDate(visit_date)))/365.0) as unit_qty,
         --(double) retail_all / EXP( ( DaysBetween(ToDate('2013-04-30','yyyy-MM-dd'),ToDate(visit_date,'yyyy-MM-dd')))/365.0) as retail_all,
         (double) retail_all / EXP( ( DaysBetween(ToDate('$today','yyyy-MM-dd'),ToDate(visit_date,'yyyy-MM-dd')))/365.0) as retail_all,
         --(double) unit_qty / EXP( ( DaysBetween(ToDate('2013-04-30','yyyy-MM-dd'),ToDate(visit_date,'yyyy-MM-dd')))/365.0) as unit_qty,
         --(double) 1.0 / EXP( ( DaysBetween(ToDate('2013-04-30','yyyy-MM-dd'),ToDate(visit_date,'yyyy-MM-dd')))/180.0) as unit_qty,
         (double) 1.0 / EXP( ( DaysBetween(ToDate('$today','yyyy-MM-dd'),ToDate(visit_date,'yyyy-MM-dd')))/180.0) as unit_qty,
         retail_per_item,
         cost_per_item,
         visit_date
...
TransOffline4 = FOREACH TransOffline3 GENERATE membership_nbr,
					 card_holder_nbr,
					CONCAT((SIZE((chararray)category_nbr) == 1 ? CONCAT('0', (chararray)category_nbr) : (chararray)category_nbr),
                        		(SIZE((chararray)sub_category_nbr) == 1 ? CONCAT('0', (chararray)sub_category_nbr) : (chararray)sub_category_nbr)) AS cat_subcat_nbr,
					unit_qty,
...
TransOffline5 = GROUP TransOffline4 BY (membership_nbr, card_holder_nbr, cat_subcat_nbr, visit_date);

TransOffline6 = FOREACH TransOffline5 GENERATE FLATTEN(group) AS (membership_nbr, card_holder_nbr, cat_subcat_nbr, visit_date),
							MAX(TransOffline4.unit_qty) AS unit_qty;
							--SUM(TransOffline4.unit_qty) AS unit_qty;

TransOffline7 = JOIN Mem4 BY (membership_nbr, cardholder_nbr) RIGHT OUTER, TransOffline6 BY (membership_nbr, card_holder_nbr); 


TransOffline8 = FOREACH TransOffline7 GENERATE TransOffline6::membership_nbr AS membership_nbr,
					 --Mem4::membership_create_date AS membership_create_date,
					 (Mem4::membership_create_date IS NULL ? '0000-00-00' : Mem4::membership_create_date) AS membership_create_date,
					 --Mem4::membership_obsolete_date AS membership_obsolete_date,
					 TransOffline6::card_holder_nbr AS cardholder_nbr, 
					 TransOffline6::cat_subcat_nbr AS cat_subcat_nbr,
...
TransOffline82 = GROUP TransOffline81 BY (membership_nbr, membership_create_date, cardholder_nbr, cat_subcat_nbr, visit_date);

TransOffline83 = FOREACH TransOffline82 GENERATE FLATTEN(group) AS (membership_nbr, membership_create_date, cardholder_nbr, cat_subcat_nbr, visit_date),
					   MAX(TransOffline81.unit_qty) AS unit_qty;
--					   SUM(TransData81.unit_qty) AS unit_qty;


--TransData83 = UNION TransOnline83, TransOffline83;
TransUnion83 = UNION TransOnline83, TransOffline83;

cardMapping = LOAD '$CardMapToActive' Using JsonLoader();

cardMapping = FOREACH cardMapping GENERATE (chararray) membershipNumber AS membership_nbr, membership_create_date AS membership_create_date, (chararray) terminalCardholder AS terminal_card, (chararray) cardholder AS card;

--left join
TransUnion83JoinCards = JOIN TransUnion83 BY (membership_nbr,membership_create_date,cardholder_nbr) LEFT, cardMapping BY (membership_nbr,membership_create_date,card); 

TransData83 = FOREACH TransUnion83JoinCards GENERATE
        TransUnion83::membership_nbr AS membership_nbr
        , TransUnion83::membership_create_date AS membership_create_date
        , (cardMapping::terminal_card IS NULL ? TransUnion83::cardholder_nbr : cardMapping::terminal_card) AS cardholder_nbr
        , TransUnion83::cat_subcat_nbr AS cat_subcat_nbr
        , TransUnion83::visit_date AS visit_date
        , TransUnion83::unit_qty AS unit_qty
;
...
TransData10 = GROUP TransData9 BY (membership_nbr, cardholder_nbr, membership_create_date, cat_subcat_nbr);

StudySet = FOREACH TransData10 GENERATE FLATTEN(group) AS (membership_nbr, cardholder_nbr, membership_create_date, cat_subcat_nbr), SUM(TransData9.unit_qty) AS total_qty;

StudySet = FOREACH StudySet GENERATE CONCAT(CONCAT(CONCAT(membership_nbr,'_'),CONCAT(cardholder_nbr,'_')),membership_create_date) as user, cat_subcat_nbr, total_qty; 
