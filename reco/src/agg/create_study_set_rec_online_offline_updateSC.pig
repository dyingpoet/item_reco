...
TransOnline82 = GROUP TransOnline81 BY (membership_nbr, membership_create_date, membership_obsolete_date, pdcardholder_nbr, cat_subcat_nbr, visit_date);

TransOnline83 = FOREACH TransOnline82 GENERATE FLATTEN(group) AS (membership_nbr, membership_create_date, membership_obsolete_date, pdcardholder_nbr, cat_subcat_nbr, visit_date),
...
TransOffline82 = GROUP TransOffline81 BY (membership_nbr, membership_create_date, membership_obsolete_date, pdcardholder_nbr, cat_subcat_nbr, visit_date);

TransOffline83 = FOREACH TransOffline82 GENERATE FLATTEN(group) AS (membership_nbr, membership_create_date, membership_obsolete_date, pdcardholder_nbr, cat_subcat_nbr, visit_date),
...
TransData10 = GROUP TransData9 BY (membership_nbr, pdcardholder_nbr, membership_create_date, cat_subcat_nbr);

StudySet = FOREACH TransData10 GENERATE FLATTEN(group) AS (membership_nbr, pdcardholder_nbr, membership_create_date, cat_subcat_nbr), SUM(TransData9.unit_qty) AS total_qty;

StudySet = FOREACH StudySet GENERATE CONCAT(CONCAT(CONCAT(membership_nbr,'_'),CONCAT(pdcardholder_nbr,'_')),membership_create_date) as user, cat_subcat_nbr, total_qty; 
