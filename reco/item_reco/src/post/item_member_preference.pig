...
Mem0 = LOAD '$MemList' USING PigStorage('\u0001') AS (
...
TransOffline0 = FILTER TransOffline00 BY unit_qty > 0 and retail_all > 0 and sub_category_nbr!=91 and sub_category_nbr!=97 and mbr_country_code=='US';
...
--TransOnline0 = FOREACH TransOnline00 GENERATE (int) order_nbr AS visit_nbr, (int) club_nbr AS club_nbr, (long) system_item_nbr, (long) 0 AS household_id, (chararray) membership_nbr, (chararray) card_holder_nbr, (int) category_nbr, (int) sub_category_nbr, (float) retail_all AS retail_all, (double) unit_qty AS unit_qty, (float) unit_retail_amt AS retail_per_item, (double) 0.0 AS cost_per_item, (chararray) order_date AS visit_date;
TransOnline0 = FILTER TransOnline00 BY unit_qty > 0 and retail_all > 0 ;
*/
...
TransOnline0 = FOREACH TransOnline00 GENERATE order_nbr AS visit_nbr, 0 AS club_nbr, system_item_nbr, membership_nbr, card_holder_nbr,category_nbr, sub_category_nbr, unit_retail_amt AS retail_all, ordered_qty AS unit_qty, order_date AS visit_date;
TransOffline0 = FOREACH TransOffline00 GENERATE visit_nbr, club_nbr, system_item_nbr, membership_nbr, card_holder_nbr,category_nbr, sub_category_nbr, retail_all, unit_qty, visit_date;
...
TransData3 = FILTER TransData2 BY (unit_qty > 0.0 AND retail_all > 0.0 AND
                                   ((visit_date >= '$DateLB' AND
visit_date <= '$DateUB') OR (visit_date >= '$SPLB' AND visit_date <= '$SPUB'))
AND
...
STORE TransData14 INTO '$OUT' USING PigStorage('\u0001');
